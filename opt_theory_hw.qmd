...
Computes an optimal charging schedule for `K` electric vehicles over `N` hours.

# Arguments
- `a`: An `N`-element vector specifying the maximum available charging energy per hour (kWh).
- `c`: An `N`-element vector representing the cost of charging per hour (€/kWh).
- `m`: A `K`-element vector with the maximum allowed charging energy for each vehicle (kWh).
- `r`: A `K`-element vector specifying the total energy required by each vehicle (kWh).
- `d`: A `K`-element vector indicating the departure time (hour) of each vehicle.
