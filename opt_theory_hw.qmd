...
# Arguments
- `a`: An `K`-element vector specifying the maximum available charging energy per hour (kWh).
- `c`: An `K`-element vector representing the cost of charging per hour (€/kWh).
- `m`: A `N`-element vector with the maximum allowed charging energy for each vehicle (kWh).
- `r`: A `N`-element vector specifying the total energy required by each vehicle (kWh).
- `d`: A `N`-element vector indicating the departure time (hour) of each vehicle.
