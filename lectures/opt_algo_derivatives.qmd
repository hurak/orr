...
#| code-fold: false
#| label: fig-fd-error
#| fig-cap: "Dependence of the absolute error of the FD approximation of the derivative on the step size $h$ for the 64- and 32-bit floating-point formats"
using Plots
...
[ForwardDiff.jl](https://github.com/JuliaDiff/ForwardDiff.jl) and [Enzyme.jl](https://github.com/EnzymeAD/Enzyme.jl) for forward AD. [Zygote.jl](https://github.com/FluxML/Zygote.jl) and [Enzyme.jl](https://github.com/EnzymeAD/Enzyme.jl) (and possibly [ReverseDiff.jl](https://github.com/JuliaDiff/ReverseDiff.jl) that is being superseded by Enzyme.jl) for reverse AD. 
There are a few more but these should certainly suffice at the beginning. 

It may be worth using the [DifferentiationInterface.jl](https://github.com/JuliaDiff/DifferentiationInterface.jl), which provides a common interface to different AD packages, and we can easily switch between them. 

If may also be useful to check also [ChainRules.jl](https://github.com/JuliaDiff/ChainRules.jl), which support writing your own rules for differentiation of custom functions.

Some more info can be found in the [discussion thread](https://discourse.julialang.org/t/state-of-ad-in-2024/112601) on [Julia Discourse](https://discourse.julialang.org). Although it is from 2024, it is still relevant.

### Matlab

Matlab has only started supporting AD relatively recently, see [Automatic Differentiation Background](https://www.mathworks.com/help/optim/ug/autodiff-background.html) and it is not intended to be used directly by users but rather as a backend for the optimization solvers.

There is, however, a very popular third party package for AD (not only) in Matlab, which is [CasADi](https://web.casadi.org). It is written in C++ but has interfaces for Matlab and Python too. Very advanced, very popular in the computational/optimal control community. 