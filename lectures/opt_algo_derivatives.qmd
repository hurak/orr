...
Note that while for the former, quite some work muse have been done before the timing was done, for the latter, we start completely from the scratch, just with the function definition. But before we start exploring the AD methods, we give a brief overview of the numerical methods based on finite-difference (FD) approximations.
:::
