...
% The key idea for deriving the necessary conditions with one of the ends lying on a curve prescribed by the function $\psi(x)$ is described\footnote{Do not be frustrated that it was also assigned as an Exercise 2.6 earlier in the book. I seriously doubt it could be solved with a modest effort without seeing the trick first.} in the proof of Weierstrass-Erdmann corner conditions in Chapter three of \cite{liberzon_calculus_2011}. 
We already discussed the terms on the left. What is new here is the term on the right. It can also be approximated by the first two terms in Taylor's expansion
$$
 \psi(b+\Delta b) = \psi(b) + \psi'(b)\Delta b.
$$

Therefore, we can expand (\ref{eq:y_should_be_on_psi_at_the_final_time}) into 
$$
...
$$
and substitute to the boundary condition (\ref{eq:boundary_conditions_free_final_time_and_state}), which after cancelling the common $\Delta b$ term yields
$$\boxed{
...
The interpretation of this result is that our desired curve $y$ hits the target curve $\phi$ in a perpendicular (transverse) direction. 
:::
