...
We have seen previously that the switching function $\lambda_2 \eqqcolon H_u$ can only be zero at isolated time instants, and that the optimal control is bang-bang, that is, it switches between the boundary values (with at most one switch). But are we sure that $\lambda_2$ cannot be zero over a non-trivial time interval? 

We learnt previously that $\lambda_2(t) = -c_1t+c_2$ and therefore $\lambda_2=0$ for a nontrivial interval implies that $c_1=0$ and $c_2=0$, which when combined with $\lambda_1(t) = c_1$ further implies that $\lambda_1=0$. But this would mean that $H = 1$, which contradicts the condition that $H=0$ along the optimal trajectory (combined implication of the free final time and time invariance). Therefore, $\lambda_2$ cannot be zero over a non-trivial time interval.
:::

This was an example in which the PMP does give a unique optimal control. The following example demonstrates that this is not always the case. 

A control that is indeterminate from the PMP conditions is called **singular control**, and the time interval on which it applies is called a **singular arc**.
