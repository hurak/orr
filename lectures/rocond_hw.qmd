...
1. Create the generalized plant $\bm{P}$,
using the transfer functions $G_1(s)$ and $G_2(s)$, and the weighting functions of your choice. The weighting functions should be designed to ensure that the closed-loop system meets the performance objectives, such as tracking.
...
3. Save the found controller $\bm{K}$ to a file name `K.jld` using the `JLD` package.
4. Compress the file `K.jld` as well as the code used to generate the controller, preferably named `hw.jl` into a zip file named `hw.zip`.
5. Submit the zip file `hw.zip` to [BRUTE](https://cw.felk.cvut.cz/brute) system.
...
K = ss(1.0I(2), 1.0I(2), 1.0I(2), 1.0I(2)) # Placeholder 

## Save the controller K to a file
save("K.jld", "K", K)
