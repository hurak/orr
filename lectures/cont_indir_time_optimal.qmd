...
$$
by checking if the function is positive or negative (or zero within some numerical tolerance).
...
function time_optimal_bang_bang_control(x, umin, umax, ϵ = 1e-3)
    y = x[1]                    # Position.
    v = x[2]                    # Velocity.
    s = y+1/2*v*abs(v)          # Switching curve s(y,v) = 0.
    if abs(s) <= ϵ              # If on the switching curve:
        if y < 0
            u = umin
        elseif y > 0
            u = umax
...
function simulate_time_optimal_double_integrator()
    ...
    uopt = time_optimal_bang_bang_control.(xopt.u,umin,umax)  # Remember that the package uses `u` as the state variable.
    return xopt, uopt
end
...
The state trajectory in the state space is in @fig-time-optimal-state-portrait, while the state and control trajectories in time are in @fig-time-optimal-control.
``` {julia}
#| code-fold: true
