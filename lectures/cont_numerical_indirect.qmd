...
\mathbf 0 &= \nabla_{\bm u} H(\bm x,\bm u,\bm \lambda)\qquad (\text{or} \qquad \bm u^\star = \text{argmin} H(\bm x^\star,\bm u, \bm\lambda^\star),\quad \bm u \in\mathcal{U})\\
...
$$f_k = f(x_k,t_k),$$ $$f_{k2} = f(x_{k2},u(t_{k2}),t_{k2}),$$ 
$$f_{k+1} = f(x_{k+1}),t_{k+1}),$$ 
...
We format the equations as *defect equations*, that is, with zero on the right hand side
$$
\begin{aligned}
...
The optimization variables for every integration interval are $x_k,x_{k2}$.
...
The optimization variables for every integration interval are the same as before, that is, $x_k,x_{k2}$.
