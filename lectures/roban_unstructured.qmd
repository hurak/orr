...
Simple (low-order) weighting filter usually suffices, such as 
$$
W_{\mathrm{p}}(s) = \frac{s/M+\omega_\mathrm{B}^*}{s+\omega_\mathrm{B}^*A},
$$
or
$$
W_\mathrm{p}(s) = \frac{(s/\sqrt{M}+\omega_\mathrm{B}^*)^2}{(s+\omega_\mathrm{B}^*\sqrt{A})^2},
$$
where $M$ is allowed peak in the sensitivity function, $\omega_\mathrm{B}^*$ is the bandwidth, that is, the frequency up to which the disturbance is attenuated, and $A$ is the attenuation of the disturbance at low frequencies. In particular, if $A=0$, the steady-state error is zero.
...
\end{bmatrix}
\right\|_{\infty}
= \sup_{\omega\in\mathbb{R}}\sqrt{|W_\mathrm{p}(j\omega)S(j\omega)|^{2}+|W(j\omega)T(j\omega)|^{2}}.
$$
