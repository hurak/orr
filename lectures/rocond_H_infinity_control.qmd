...
Conditions for existence of $\bm K$ minimizing $\|F_l(P,K)\|_{\infty}$ are:

- the system $(\mathbf A, \mathbf B_2, \mathbf C_2)$ is stabilizable and detectable,
- the matrix $\mathbf D_{12}$ has a full column rank and the matrix $\mathbf D_{21}$ has a full row rank, otherwise the controller has high gain at high frequency (nonproper),
- the matrix $\begin{bmatrix}\mathbf A-j\omega I & \mathbf B_2\\ \mathbf C_1 & \mathbf D_{12}\end{bmatrix}$ full column rank for all $\omega$,
- the matrix $\begin{bmatrix}\mathbf A-j\omega I & \mathbf B_1\\ \mathbf C_2 & \mathbf D_{21}\end{bmatrix}$ full row rank for all $\omega$ (to prevent low damped poles of the controller),
