...
$$
which turns the difference Lyapunov equation into the even more famous **(algebraic) Lyapunov equation** 
$$
\boxed{\mathbf S = \mathbf A^\top \mathbf S\mathbf A+\mathbf Q.}
$$

The following facts about this equation are typically studied in courses on linear systems (and that is why we omit their proofs here):

- If $\mathbf A$ stable and $\mathbf Q = \mathbf Q^\top$ , then there is a unique solution $\mathbf S = \sum_{k=0}^\infty \left(\mathbf A^\top\right)^k \mathbf Q \mathbf A^k$ .
- If $\mathbf A$ stable and $\mathbf Q\succeq 0$ , then there is a unique solution $\mathbf S\succeq 0$ .
- If $\mathbf A$ stable and $(\mathbf A,\sqrt{\mathbf Q})$ observable, then there is a unique solution $\mathbf S\succ 0$ .
	- Furthermore, if $\mathbf A$ stable and there is a solution $\mathbf S\succ 0$ , then $(\mathbf A,\sqrt{\mathbf Q})$ is observable.
...
This is a superfamous equation and is called **discrete-time Riccati equation** or **Riccati difference equation** or **discrete-time Riccati recursion**. When initialized with $\mathbf S_N$, it generates the sequence of matrices $\mathbf S_{N-1}, \mathbf S_{N-2}, \mathbf S_{N-3},\ldots$ Indeed, a noteworthy feature of this sequence is that it is initialized at the final time and the equation prescribes how the sequence evolves backwards.
...
The last output suggests that both $s_N$ and $k_k$ stay constant for most of the time interval and they only change dramatically towards the end of the control interval.
