...
Leaving aside for the moment the important question whether and under which conditions such a limit $\mathbf S_\infty$ exists, the immediate question is how to compute such limit. One straightforward strategy is to evaluate the Riccati recursion and generate the sequence $\mathbf S_{N}, \mathbf S_{N-1}, \mathbf S_{N-2}, \ldots$ so long as there is a nonnegligible improvement, that is, once $\mathbf S_{k}\approx\mathbf S_{k+1}$, stop iterating. That is certainly doable.   
...
## Boundedness of the solution to the Riccati recursion

Let us state the answer first: the system $\bm x_{k+1}=\mathbf A \bm x_k+\mathbf B \bm u_k$ must be stabilizable in order to guarantee existence of a bounded limiting solution $\mathbf S_\infty$ to the Riccati recursion.
...
The last issue that we have to solve is *uniqueness*. Why do we need to care? We have already discussed that the ARE, being a quadratic equation, can have more than just one solution. 

In the scalar case it can have two real solutions (or no real solution because both are complex). These are the possibilities:

- None of the solutions is nonnegative. Bad luck, no stabilizing solution can be found. This can be, however, excluded if the system $(\mathbf A,\mathbf B)$ is stabilizable as discussed before.
- Only one of them is nonnegative. We are lucky because this is our stabilizing solution.  
- Both solutions are nonnegative. But then only one of them can correspond to the unique solution of the Riccati recursion, hence only one of them is guaranteed to yield a stabilizing feedback controller. 

We now ask if we can exclude the last scenario – multiple nonnegative (or positive semidefinite in the general matrix case) solutions of ARE. In other words, we are asking if the positive semidefinite stabilizing solution to ARE is unique. We will use ARE in the Joseph stabilized form
$$
...
Observe that for a fixed (optimal) $\mathbf K$ the above equation is actually a Lyapunov equation for an equivalent system with the state matrices $(\mathbf A-\mathbf B\mathbf K,\begin{bmatrix}\mathbf C\\ \mathbf D\mathbf K\end{bmatrix})$. We have previously refreshed the well-known fact about Lyapunov equation that if the system is stable (which it is in our case since $(\mathbf A-\mathbf B\mathbf K)$ matrix is) and observable, the equation has a unique positive definite solution. We only state it here that if the observability is relaxed to detectability, existence of a unique positive definite solution is guaranteed instead.
