...
$$
 J^\mathrm{aug}(t,\bm x(\cdot),\dot{\bm x}(\cdot),\bm u(\cdot),\bm\lambda(\cdot)) = \int_{t_\mathrm{i}}^{t_\mathrm{f}}\left[\underbrace{ L(\bm x,\bm u,t)+\bm\lambda^\top\left(\mathbf f(\bm x,\bm u,\mathbf t)-\dot {\bm x}\right)}_{L^\text{aug}}\right ]\text{d}t,
$$ {#eq-augmented-cost-function}
in which we have also identified the augmented Lagrangian 
$$
L^\text{aug}(\bm x,\bm u,\dot{\bm x},\bm\lambda,t) = L(\bm x,\bm u,t)+\bm\lambda^\top\left(\mathbf f(\bm x,\bm u,\mathbf t)-\dot {\bm x}\right).
...
\mathbf{f}(\bm x,\bm u,t) - \dot{\bm{x}} &= \mathbf 0.
\end{aligned}
$$
...
\mathbf{f}(\bm x,\bm u,t) - \dot {\mathbf{x}} &= \mathbf 0.
\end{aligned}
$$
...
We now introduce an auxilliary function called *Hamiltonian*, which not only makes the conditions of optimality even more compact, but also – and even more importantly – it strengthens the link with the physics-motivated calculus of variations. We expand the definition of the augmented Lagrangian 
$$
\begin{aligned}
L^\mathrm{aug}(\bm x,\bm u,\bm \lambda,t) &= L(\bm x,\bm u,t)+\bm \lambda^\top\left(\mathbf f(\bm x,\bm u,\mathbf t)-\dot {\bm x}\right)\\
&= L(\bm x,\bm u,t)+\bm \lambda^\top \mathbf f(\bm x,\bm u,\mathbf t) - \bm \lambda^\top \dot {\bm x}. 
\end{aligned}
$${#eq-augmented-lagrangian-expanded} 
and label the first two terms as $H(\bm x,\bm u,\bm\lambda,t)$, that is, we define
$$\boxed{
H(\bm x,\bm u,\bm\lambda,t) \coloneqq L(\bm x,\bm u,t)+\bm \lambda^\top \mathbf f(\bm x,\bm u,\mathbf t).}
$${#eq-hamiltonian}

Using this new function, the necessary conditions of optimality in @eq-first-order-conditions-optimal-control-nonlinear-vector can be rewritten in a more compact form:
$$\boxed{
\begin{aligned}
...
Now, I promised some connection with the (physics-motivated) calculus of variations. Recall that the Hamiltonian function in the calculus of variations is defined as $H(x,y,y',p) = py'-L$. At first sight, this does not seem to resemble the function we have just defined in @eq-hamiltonian for optimal control. First, the control Hamiltonian does not explicitly depend on the derivative of any function. But realizing that through the state equation $\dot{\bm x}=\mathbf f(\bm x,\bm u, t)$, the derivative $\dot{\bm x}$ at a given state $x$ (and time $t$) is uniquelly determined by $u$, we can consider the control-related Hamiltonian as a function of just $t, \mathbf x, \mathbf u$ and $\bm\lambda$ (and discard the $\dot{\bm x}$ argument). Second, in the optimal control setting we consider constraints in the form of the state equation, hence we need to use the augmented Lagrangian, which is related to the Hamiltonian function as
$$
H(\bm x,\bm u,\bm \lambda, t) = \bm \lambda^\top\dot {\bm x} + L^\mathrm{aug}(\bm x,\bm u,\bm \lambda,t).
$$

This is nearly it, barring the sign in front of the Lagrangian (and that the new independent variable is $t$ while it was $x$ in the calculus of variations, and that the new dependent variables are $\bm x$ and $\bm u$ and $\bm \lambda$, the latter playing the the role of $p$, is perhaps obvious). We could leave it at it, with the comment that the optimal control has just a different sign convention. 

But if you want to dig deeper, note that our choice of the constraint function in the augmented Lagrangian @eq-augmented-lagrangian was somewhat arbitrary – we could have defined it as $\dot {\bm x}-\mathbf f(\bm x,\bm u,\mathbf t)$ instead of $\mathbf f(\bm x,\bm u,\mathbf t)-\dot {\bm x}$, which would have resulted in a different augmented Lagrangian, namely 
$$
\tilde L^\mathrm{aug}(\bm x,\bm u,\bm \lambda,t) = L(\bm x,\bm u,t)+\bm \lambda^\top\left( \dot {\bm x}-\mathbf f(\bm x,\bm u,\mathbf t)\right).
$$ 

If we now defined the Hamiltonian function as 
$$
\tilde H(\bm x,\bm u,\bm\lambda,t) \coloneqq \bm\lambda^\top \mathbf f(\bm x,\bm u,t) - L(\bm x,\bm u,t),
$${#eq-hamiltonian-alternative}
we could relate it to the augmented Lagrangian as
$$
\tilde H(\bm x,\bm u,\dot{\bm{x}},\bm\lambda,t) = \bm\lambda^\top \, \dot{\bm x} - \tilde L^\text{aug},
$$
whis now matches the structure of the Hamiltonian even more closely. But this is all just a matter of a sign convention, and the fact is that the former definition of the control Hamiltonian in @eq-hamiltonian is more common in the optimal control literature, so we stick to it. 
