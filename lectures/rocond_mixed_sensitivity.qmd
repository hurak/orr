...
Let's introduce the first instance of such methodology. We have learnt that the robust performance condition in presence of multiplicative uncertainty is formulated as an upper bound on the $\mathcal H_\infty$ norm of the *mixed sensitivity function* $\begin{bmatrix}W_\mathrm{p}S\\WT\end{bmatrix}$, namely
$$
\left\|
\begin{bmatrix}
W_\mathrm{p}S\\WT
...
The middle term penalizes the control (it is the closed-loop transfer function from the output disturbance to the control). It resembles the role of the $\mathbf R$ term in the LQR cost $\int(\bm x^\top \mathbf Q\bm x+\bm u^\top \mathbf R\bm u)\text{d}t$. Typically we set $W_2$ equal to a small  nonnegative constant.

An important property of this method is that it extends to the multiple-input-multiple-output (MIMO) case. Nothing needs to be changed in the formal problem statement as the $\mathcal H_\infty$ norm is defined for MIMO systems as well. For completeness, we state the MIMO version of the problem (the statement only differs in the fonts)
...
If you are not familiar with these and feel uneasy about it, you can consult the original paper. But for our purposes in this lecture we do not rely on a domain-specific expertize – we view these variables as generic inputs and outputs of a MIMO system.

We proceed by definining the (model of the) system:
``` {julia}
#| code-fold: show
#| output: false
...
Now, we form the generalized plant, check if the solvability conditions are satisfied, and call the solver of the general $\mathcal H_\infty$ optimal control problem: 
``` {julia}
#| code-fold: false
...
Finally, we call the solver of the general $\mathcal H_\infty$ optimal control problem:
``` {julia}
#| code-fold: false
#| output: false
...
The second input argument for the `hinfsynthesize` solver requires some explanation. Solvers for $\mathcal H_\infty$ optimization problems typically implement the so-called $\gamma$ iteration, which means that they start with some value of $\gamma$, attempt to solve the problem of searching for a stabilizing controller with which the $\mathcal H_\infty$ norm of the corresponding closed-loop transfer function satisfies the upper bound $\|\cdot\|_\infty < \gamma$, if it succeeds, it tries the same for a smaller $\gamma$, if it fails, it increases $\gamma$ and tries again. This is then repeated until the smallest possible $\gamma$ is found for which the problem is solvable. The `γrel` input argument specifies how much larger than the smallest possible $\gamma$ we want to be. Setting `γrel=1.05` means that we want to be 5% larger than the smallest possible $\gamma$. It is typically not a good idea to aim at achieving the smallest possible $\gamma$ (the optimal solution) because the existing methods experience numerical issues as they approach the optimal solution. Here the achieved (suboptimal) $\gamma$ is
``` {julia}
#| code-fold: false
γ
```

Recall that we developed our robust performance condition as the upper-boundedness of the norm by some constant (at least in the SISO case). Here we apparently violate the condition. But it is up to us to decide whether the resulting closed-loop responses are satisfactory. The minimization of $\gamma$ is just a tool to shape the closed-loop magnitude frequency responses. After all, now that we also included also the $\mathbf{KS}$ in the mixed sensitivity function, the particular value of $\gamma$ has no longer a direct interpretation.

Finally, we form the three closed-loop sensitivity functions.
``` {julia}
#| code-fold: false
#| output: false
