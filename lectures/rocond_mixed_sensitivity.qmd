...
The magnitude (actually the largest singular values) frequency responses of the closed-loop transfer functions of interest are shown in @fig-himat-mixed-sensitivity.
``` {julia}
...
The responses to steps in the reference value of the two angles are shown in @fig-himat-mixed-sensitivity-step-response-alpha.
``` {julia}
...
T = named_ss(T, u=[:α_ref, :θ_ref], y =[:α, :θ])
step_response = step(T)
#plot(step_response, title="", xlabel="Time (s)", ylabel=["α" "θ"], lw=2, label=["α_ref", "θ_ref"], legend=:topright)
plot(step_response, title="", xlabel="Time (s)", lw=2, legend=:topright)
```
:::