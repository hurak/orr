...
$$\boxed{
J(\ldots) = \frac{1}{2} \sum_{k=0}^{t_\mathrm{final}} \left((\bm x_{k}-{\color{blue}\mathbf x^\mathrm{ref}})^\top \mathbf Q \, (\bm x_{k}-{\color{blue}\mathbf x^\mathrm{ref}}) + \bm u_{k}^\top \mathbf R \bm u_{k}\right).}
...
Standard double-mass-spring-damper system. Fourth-order system with one control input (force acting on the first mass) and one output (the position of the second mass).
```{julia}
#| code-fold: show
#| output: false
...
# Example 8.3 (double-mass-spring-damper system), page 287, Digital Control of Dynamic Systems, Franklin, Powell, Emami-Naeini, 3rd edition.
M = 1
m = 0.1
b = 0.036
k = 0.091
...
p2 = plot(res, ylabel="State", lab=["y dot" "y" "d dot" "d"], ploty=false, plotx=true, plotu=false, layout=1, sp=1, lw=2, seriestype=:steppost)
...
The control law is given by
$$\boxed{
\bm u_k = - \begin{bmatrix}\mathbf K & \mathbf K^\mathrm{I}\end{bmatrix} \begin{bmatrix}\bm x_k \\ \bm x_k^\mathrm{I}\end{bmatrix} + \mathbf K \mathbf N_\mathrm{u} \mathbf y^\mathrm{ref}.}
$$
