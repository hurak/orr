...
$$
\begin{align*}
    \underset{\mathbf{x}, u}{\text{minimize}} \quad & \frac{1}{2}\sum_{k=t}^{t+N-1} (\mathbf{y}_{k+1} - r_{k+1})^T\mathbf{Q}(\mathbf{y}_{k+1} - r_{k+1}) + \Delta\mathbf{u}_k^T\mathbf{R}\Delta\mathbf{u}_k\\
...
$$
\begin{array}{rl}
\underset{\mathbf{x}, u}{\text{minimize}} \quad &  \frac{1}{2}\mathbf{z}^T \mathbf{H} \mathbf{z} + [\mathbf{x}_t^T \: \mathbf{u}_{t-1}^T \: \mathbf{r}_{t+1,\dots,t+N}^T]\,\mathbf{F}\,\mathbf{z} \\
...
function setup_mpc(A::Matrix{T}, B::Matrix{T}, C::Matrix{T}, Q::Matrix{T}, R::Matrix{T}, N::Int, u_min::Vector{T}, u_max::Vector{T}) where T <: AbstractFloat
    ...
    H = zeros(T, N * nu, N * nu)
    F = zeros(T, (nx + nu + N * ny) * N, N * nu)
    S = zeros(T, N * nu, nx + nu)
    W = zeros(T, N * nu)
    G = zeros(T, N * nu, N * nu) 
...
function MPCProblem(A::Matrix{T}, B::Matrix{T}, C::Matrix{T}, Q::Matrix{T}, R::Matrix{T}, N::Int, u_min::Vector{T}, u_max::Vector{T}) where T <: AbstractFloat
    ...
    x₀ = zeros(T, size(A, 1)) # Dummy initial state
    r = zeros(T, N * size(C, 1)) # Dummy reference trajectory
    u₀ = zeros(T, size(B, 2)) # Dummy initial input
    ...
    constr = COSMO.Constraint(Ac, bc, COSMO.Nonnegatives);

    settings = COSMO.Settings(verbose=true);

    COSMO.assemble!(model,sparse(H), q, constr, settings = settings) # Assemble the QP
...
function solve!(mpc::MPCProblem{T}, xₖ::Vector{T}, uₖ₋₁::Vector{T}, r::Matrix{T}) where T <: AbstractFloat
    ...
    # TODO implement the updates for the COSMO model (q and b)
    q = zeros(T, size(mpc.F, 2))
    b = zeros(T, size(mpc.S, 1))

    COSMO.update!(mpc.model, q = q, b = b)
...
You may want to test your implementation using the following example:
```{julia}
#| eval: false
...
using Plots
plot(t, ys[1, :], label="y1")
plot!(t, ys[2, :], label="y2")

plot!(t, ref[1, 1:end-N], linestyle=:dash, label="ref1")
plot!(t, ref[2, 1:end-N], linestyle=:dash, label="ref2")
