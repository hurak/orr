...
![Model of a plant with a disturbance model and a reference. The block R represents a scaling gain that is generally needed for scaling.](figures/plant_with_disturbance_model.png){#fig-plant-with-disturbance-model width=40%}
...
We formally state the result as
$$\boxed{
...
bodeplot(G,plotphase=false, label="G", lw=2)
bodeplot!(Gd, plotphase=false, label="Gd", lw=2)
```
...
k = 0.8                             # should be less than 1
Gd = k*tf(1,[1, 1])*tf(1,[0.2, 1])
