...
Let $\mathbf{x} = (x, \dot{x}, \theta, \dot{\theta})$ be the state of the system. Your objective is to drive the system from the initial state ${\mathbf{x}_\text{initial} = (0, 0, 0, 0)}$ (cart at rest at the origin, pole hanging down) to the final state ${\mathbf{x}_\text{final} = (0, 0, \pi, 0)}$ (cart at rest at the origin, pole upright).
...
- Return the planned trajectory as
    ...
    where `N` corresponds to the number of simulation steps (e.g., `N = 1000` for 10 seconds with time step `Δt = 0.01`).
...
Your controller and planner will be used in the BRUTE system in a similar way as the snippet below.
