...
Since in our quest for the optimal control we optimize over functions (trajectories), we can view our optimization as cast in an infinite-dimensional vector space. The mathematical discipline of *calculus of variations* provides concepts and tools for such optimization. The general task (in a scalar version) is to do the following optimization
$$ 
\operatorname*{minimize}_{y(\cdot)\in\mathcal C^1[a,b]} J(y(\cdot)),
$$
...
::: {#exm-strong-vs-weak}
Consider the minimization of the functional $J(y)=\int_\mathrm{a}^\mathrm{b}[(y'(x))^2(1-(y'(x))^2)]\text{d}x$ for which it is requested that $y(a)=y(b)=0$. Clearly $y(x)=0$ is a weak minimum but is not a strong minimum. Just observe that even for tiny perturbations in magnitude, if the derivative is high, that is, $(y'(x))^2>1$, the functional $J$ is negative. 
:::
...
$$
\begin{aligned}
 \operatorname*{maximize}_{y\in\mathcal{C}^1_{[a,b]}} &\quad \int_a^b y(x)\text{d}x,\\
...
$$
 y(a) = \mathrm y_a,\qquad y(b) = \mathrm y_b,
$$
...
Identically as in our previous development, we can get rid of the derivative of the variation using integration *per partes*, which gives
$$\boxed{
