...
\end{aligned}
$$
where the latter constraint is more often than not written as $\bm X \succeq 0$, understanding from the context that the cone of symmetric positive definite matrices is assumed.
...
$$
\mathbf F_0 + \sum_{i=1}^n \mathbf F_i \bm X_i \mathbf G_i  \succ 0.
$$

A prominent example of an LMI in the context of control theory is the Lyapunov inequality for assymptotic stability of a linear time-invariant system $\dot{\bm x} = \mathbf A \bm x$, specifically, $\mathbf A^\top \bm X + \bm X \mathbf A \prec 0$, where the optimization variable is the symmetric matrix $\bm  X = \bm X^\top$ and it is constrained to be positive definite, that is, $\bm X \succ 0$. To summarize, the two linear matrix inequalities that give conditions of asymptotic stability of an LTI system are
$$
\begin{aligned}
\bm X &\succ 0,\\
